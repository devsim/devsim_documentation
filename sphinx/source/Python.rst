...
This option is only required on systems, such as |mswindows|, which do not default to this encoding.  Care should be taken when using ``unicode`` names for visualization using the tools in :ref:`ch__visualization`, as this character set may not be supported.
